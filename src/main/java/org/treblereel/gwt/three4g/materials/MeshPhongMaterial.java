 ...
 * <p>
 * The material uses a non-physically based Blinn-Phong model for calculating reflectance. Unlike the Lambertian model
 * used in the MeshLambertMaterial this can simulate shiny surfaces with specular highlights (such as varnished wood).
 * <p>
 * Shading is calculated using a Phong shading model. This calculates shading per pixel (i.e. in the fragment shader,
 * AKA pixel shader) which gives more accurate results than the Gouraud model used by MeshLambertMaterial, at the
 * cost of some performance. The MeshStandardMaterial and MeshPhysicalMaterial also use this shading model.
 * <p>
